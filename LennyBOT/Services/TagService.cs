    ...
    public static void CreateTag(string name, string content, ulong ownerId)
        {
            ...
            File.WriteAllText("Files/tags.json", jsonString);
        }
        ...
        public static Tag GetTag(string name)
        {
            var jsonString = File.ReadAllText("Files/tags.json");
            var arr = JsonConvert.DeserializeObject<Tag[]>(jsonString);
            var tag = arr.FirstOrDefault(t => string.Equals(t.Name, name, StringComparison.InvariantCultureIgnoreCase));
            return tag;
        }
        ...
        public static bool RemoveTag(Tag tagToRemove)
        {
            ...
            var arr = JsonConvert.DeserializeObject<Tag[]>(jsonString);
            var list = arr.ToList();
            var success = list.Remove(list.Find(t => string.Equals(t.Name, tagToRemove.Name, StringComparison.InvariantCultureIgnoreCase)));
            if (!success)
            {
                return false;
            }
