...
namespace LennyBOT.Modules
{
    ...
    public class SearchModule : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task SteamUserCmdAsync([Remainder] string user = null)
        {
            if (user == null)
            {
                await this.ReplyAsync("`You did not specify user`\nHow to get steam user? http://i.imgur.com/pM9dff5.png").ConfigureAwait(false);
                return;
            }
            ...
            try
            {
                ...
                var steamUser = (await SteamWebAPI.General().ISteamUser().ResolveVanityURL(user).GetResponseAsync().ConfigureAwait(false)).Data.Identity;
                if (steamUser == null)
                {
                ...
                var badges = await SteamWebAPI.General().IPlayerService().GetBadges(steamUser).GetResponseAsync().ConfigureAwait(false);
                var lastPlayed = await SteamWebAPI.General().IPlayerService().GetRecentlyPlayedGames(steamUser).GetResponseAsync().ConfigureAwait(false);
                var friends = await SteamWebAPI.General().ISteamUser().GetFriendList(steamUser, RelationshipType.Friend).GetResponseAsync().ConfigureAwait(false);
                ...
                if (lastPlayed.Data.TotalCount > 0)
                {
                    ...
                    gamePlay1 = (int)Math.Ceiling(lastPlayed.Data.Games[0].PlayTimeTotal.TotalHours)
                                + " hours";
                }
                ...
                if (lastPlayed.Data.TotalCount > 1)
                {
                    ...
                    gamePlay2 = (int)Math.Ceiling(lastPlayed.Data.Games[1].PlayTimeTotal.TotalHours)
                                + " hours";
                }
                ...
                if (lastPlayed.Data.TotalCount > 2)
                {
                    ...
                    gamePlay3 = (int)Math.Ceiling(lastPlayed.Data.Games[2].PlayTimeTotal.TotalHours)
                                + " hours";
                }
                ...
                if (lastPlayed.Data.TotalCount > 3)
                {
                    ...
                    gamePlay4 = (int)Math.Ceiling(lastPlayed.Data.Games[3].PlayTimeTotal.TotalHours)
                                + " hours";
                }
                ...
                if (lastPlayed.Data.TotalCount > 4)
                {
                    ...
                    gamePlay5 = (int)Math.Ceiling(lastPlayed.Data.Games[4].PlayTimeTotal.TotalHours)
                                + " hours";
                }
                ...
                var builder = new EmbedBuilder()
                    ...
                    .WithUrl("https://steamcommunity.com/id/" + user)
        ...
        public async Task SteamInventoryCmdAsync(string steamId64)
        {
            ...
            {
                var result = await webClient.GetAsync($"http://csgobackpack.net/api/GetInventoryValue/?id={steamId64}").ConfigureAwait(false);
        ...
        public async Task OsuCmdAsync(string user = null)
        {
            ...
            try
            {
                ...
                using (var webClient = new HttpClient())
                {
                    var result = await webClient.GetAsync($"https://osu.ppy.sh/api/get_user?k={osuApiKey}&u={user}").ConfigureAwait(false);
                    var jsonContent = await result.Content.ReadAsStringAsync().ConfigureAwait(false);
                ...
                var embed = new EmbedBuilder()
                    ...
                    .WithCurrentTimestamp()
                    .AddField("Stats", $"```json\nPlayed songs: {osuData?.Playcount}\n    Accuracy: {string.Format("{0:F2}", acc).Replace(',', '.')}```", true)
                    .AddField("Score", $"```json\n Total: {osuData?.Total_score}\nRanked: {osuData?.Ranked_score}```", true)
                    .AddField("Performance points", $"```json\nPerformance points: {osuData?.Pp_raw}\nWorldwide rank: {osuData?.Pp_rank}\nCountry rank: {osuData?.Pp_country_rank} ({osuData?.Country})```", true)
                    .AddField("Rank", $"```json\nSS: {osuData?.Count_rank_ss}\n S: {osuData?.Count_rank_s}\n A: {osuData?.Count_rank_a}```", true)
                    .Build();
                await this.ReplyAsync(string.Empty, false, embed).ConfigureAwait(false);
            }
            catch (Exception)
